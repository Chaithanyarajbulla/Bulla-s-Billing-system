############################################CREATED BY Chaithanya Raj Bulla#################################
from tkinter import *
import random
...
class Bill_App:
    def __init__(self,root):
        ...
        lbl_color = 'white'
        #Title of App
        title = Label(self.root,text = "Bulla's Billing System",bd = 12,relief = GROOVE,fg = fg_color,bg = bg_color,font=("times new roman",30,"bold"),pady = 3).pack(fill = X)
...
#Function For Text Area
    def welcome_soft(self):
        ...
        self.txt.insert(END,"       Welcome To Bulla's Mart\n")
...
#Add Product name , qty and price to bill area
    def bill_area(self):
        ...
        if self.salt.get() != 0:
            self.txt.insert(END,f"\nSalt             {self.salt.get()}           {self.salt.get() * 170}")
        if self.maza.get() != 0:
